...
A fill-in-the-blanks kit for building the Resume Update System with Claude. You give Claude a job description and a base resume. It grades the resume like a strict ATS, recommends edits pulled only from your own experience bank, builds a tailored one-page resume once you approve, and logs the application to your tracker (Notion or Google Drive).
...
In your repo on GitHub, go to **Settings → Pages**. Under *Build and deployment*, choose **Deploy from a branch**, then pick `main` and the `/docs` folder, and save. After a minute or two the playbook is live at `https://<your-username>.github.io/resume-update-system/`.
